-- Atomic, lease-based claim of pending device commands.
-- Prevents the same command being handed to a device twice on concurrent /commands polls.
...
BEGIN
  -- 1. Auto-expire stale pending commands (never execute a noon fan_off at 3 AM)
  ...
  -- 2. Atomically claim fresh, unleased commands and return them
  RETURN QUERY
  WITH candidates AS (
    SELECT dc.id
