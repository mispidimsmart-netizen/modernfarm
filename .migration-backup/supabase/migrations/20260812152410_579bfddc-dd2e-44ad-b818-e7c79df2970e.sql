...
BEGIN
  ...
  ELSE
    -- Layer feed: prefer rows explicitly tagged to this batch; only fall back to
    -- date-range rows when the farm has no batch-tagged feed at all.
    SELECT count(*) INTO tagged FROM public.feed_consumption c WHERE c.farm_id = farm.id AND c.batch_id IS NOT NULL;
    IF tagged > 0 THEN
      SELECT COALESCE(jsonb_agg(jsonb_build_object('date', c.consumption_date, 'feed_type', c.feed_type, 'quantity_kg', c.quantity_kg) ORDER BY c.consumption_date), '[]'::jsonb)
        INTO feed_rows FROM public.feed_consumption c WHERE c.batch_id = batch.id;
    ELSE
      SELECT COALESCE(jsonb_agg(jsonb_build_object('date', c.consumption_date, 'feed_type', c.feed_type, 'quantity_kg', c.quantity_kg) ORDER BY c.consumption_date), '[]'::jsonb)
        INTO feed_rows FROM public.feed_consumption c
        WHERE c.farm_id = farm.id AND c.batch_id IS NULL AND c.consumption_date BETWEEN b_start AND b_end;
    END IF;
  END IF;

  -- Medicine: prefer batch-tagged rows; fall back to untagged farm rows only when
  -- the farm never tags medicine to batches (avoids layer/broiler cross-over).
  SELECT count(*) INTO tagged FROM public.medicine_usage m WHERE m.farm_id = farm.id AND m.batch_id IS NOT NULL;
  IF tagged > 0 THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object('date', m.usage_date, 'name', m.medicine_name, 'type', m.medicine_type) ORDER BY m.usage_date), '[]'::jsonb)
      INTO med_rows FROM public.medicine_usage m WHERE m.batch_id = batch.id;
  ELSE
    SELECT COALESCE(jsonb_agg(jsonb_build_object('date', m.usage_date, 'name', m.medicine_name, 'type', m.medicine_type) ORDER BY m.usage_date), '[]'::jsonb)
      INTO med_rows FROM public.medicine_usage m
      WHERE m.batch_id IS NULL AND m.farm_id = farm.id AND m.usage_date BETWEEN b_start AND b_end;
  END IF;
