...
export function useScheduleNotifications() {
  ...
  const query = useQuery({
    queryKey: ['schedule_notifications', user?.id, selectedFarmId],
    queryFn: async () => {
      if (!user) return [];
      let q = supabase
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`schedule-notifications-${selectedFarmId ?? user.id}`)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'schedule_notifications', filter },
        () => {
