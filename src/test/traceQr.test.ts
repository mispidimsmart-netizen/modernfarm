...
describe('buildTraceUrl', () => {
  it('always uses the production domain, never preview/localhost', () => {
    const url = buildTraceUrl({ slug: 'abc123', kind: 'layer', batchId: 'b-1' });
  ...
  it('embeds kind and batch id', () => {
    expect(buildTraceUrl({ slug: 's1', kind: 'broiler', batchId: 'B-9' })).toBe(
      'https://farmeye.pro.bd/trace/s1?kind=broiler&batch=B-9',
    );
  });

  it('rejects missing or invalid inputs', () => {
    expect(() => buildTraceUrl({ slug: '', kind: 'layer', batchId: 'b' })).toThrow();
    expect(() => buildTraceUrl({ slug: 's', kind: 'duck' as never, batchId: 'b' })).toThrow();
    expect(() => buildTraceUrl({ slug: 's', kind: 'layer', batchId: '' })).toThrow();
  });
});

describe('parseTraceUrl', () => {
  it('accepts a freshly generated link round-trip', () => {
    const url = buildTraceUrl({ slug: 'xyz', kind: 'layer', batchId: 'batch-7' });
    expect(parseTraceUrl(url)).toEqual({ valid: true, slug: 'xyz', kind: 'layer', batchId: 'batch-7' });
  });
