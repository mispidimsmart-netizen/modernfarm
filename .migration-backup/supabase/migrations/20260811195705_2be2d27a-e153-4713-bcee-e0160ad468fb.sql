...
BEGIN
  ...
  SELECT id, name, name_en, location, photo_url, owner_id, created_at, total_sheds, organization_id
  ...
  RETURN jsonb_build_object(
    ...
    'generated_at', now(),
    'brand_name', COALESCE(org_name, farm.name),
    'farm', jsonb_build_object(
      'id', farm.id,
      'code', UPPER(LEFT(REPLACE(farm.id::text, '-', ''), 8)),
      'reg_no', 'FE-' || UPPER(LEFT(REPLACE(farm.id::text, '-', ''), 6)) || '-' || to_char(farm.created_at, 'YY'),
      'name', farm.name,
