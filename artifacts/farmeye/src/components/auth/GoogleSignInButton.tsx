...
interface Props {
  ...
  showDivider?: boolean;
  /** Relative same-origin path to return to after OAuth. Defaults to "/". */
  nextPath?: string;
}
    ...
    try {
      ...
      if (error) {
        toast({
          ...
          description: error.message || (language === 'bn' ? 'Google সাইন-ইন ব্যর্থ' : 'Google sign-in failed'),
          variant: 'destructive',
        });
        setLoading(false);
        return;
      }
      // Supabase redirects the browser to the configured Google OAuth provider.
    } catch (e) {
      toast({
        title: language === 'bn' ? 'ত্রুটি' : 'Error',
